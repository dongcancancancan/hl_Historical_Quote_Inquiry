...
CREATE TABLE quotation_calculation_run (
	id SERIAL NOT NULL,
	quotation_main_id INTEGER NOT NULL,
	bpm_instance_id INTEGER,
	quotation_code VARCHAR NOT NULL,
	bpm_no VARCHAR,
	run_type VARCHAR,
	status VARCHAR,
	...
	PRIMARY KEY (id),
	FOREIGN KEY(quotation_main_id) REFERENCES quotation_main (id),
	FOREIGN KEY(bpm_instance_id) REFERENCES quotation_bpm_instance (id)
);
COMMENT ON COLUMN quotation_calculation_run.bpm_instance_id IS '本次计算所属 BPM 实例';
COMMENT ON COLUMN quotation_calculation_run.params_snapshot IS '本次计算参数快照，如铜价、铜杆加工费、增值税率';
COMMENT ON COLUMN quotation_calculation_run.is_adopted IS '是否被已报价快照采用';

-- quotation_calculation_trace 在原字段基础上扩展：
-- run_id, bpm_instance_id, entity_type, entity_id, display_label, skill_id, cell_key, depends_on, source_refs

-- ====== 已报价快照表：标记已报价时固化整张报价表 ======

CREATE TABLE quotation_quote_snapshot (
	id SERIAL NOT NULL,
	quotation_main_id INTEGER NOT NULL,
	bpm_instance_id INTEGER NOT NULL,
	...
	FOREIGN KEY(quotation_main_id) REFERENCES quotation_main (id),
	FOREIGN KEY(bpm_instance_id) REFERENCES quotation_bpm_instance (id),
	FOREIGN KEY(calculation_run_id) REFERENCES quotation_calculation_run (id)
);
COMMENT ON COLUMN quotation_quote_snapshot.snapshot_data IS '已报价时整张成本分析表 JSON 快照，后续主表变化不影响该快照';
COMMENT ON COLUMN quotation_quote_snapshot.calculation_run_id IS '该快照采用的计算批次，用于查看当时计算过程';
