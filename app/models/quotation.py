...
class QuotationMain(Base):
    ...
    monthly_interest = Column(Numeric(18, 10), comment="月结利息率")
...
class QuotationBpmInstance(Base):
    ...
    vat_rate = Column(Numeric(18, 4), comment="本次增值税率")
    ul_label_fee = Column(Numeric(18, 4), comment="本次UL标签费")
    transport_fee = Column(Numeric(18, 4), comment="本次运输费")
    other_fee = Column(Numeric(18, 4), comment="本次其他费用")
    net_profit_rate = Column(Numeric(18, 4), comment="本次净利率")
    customs_fee = Column(Numeric(18, 4), comment="本次报关费")
    order_meterage = Column(Numeric(18, 4), comment="本次订单米数")
    operating_expense_rate = Column(Numeric(18, 4), comment="本次营业费用率")
    monthly_interest = Column(Numeric(18, 10), comment="本次月结利息率")
    corporate_tax_rate = Column(Numeric(18, 4), comment="本次企业所得税税率")
    ...
    material_ratio = Column(Numeric(18, 4), comment="本次材料占比")
    order_weight = Column(Numeric(18, 4), comment="本次订单重量KG")
